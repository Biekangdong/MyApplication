...
public class ColorPickerView extends FrameLayout implements LifecycleObserver {
    ...
    private boolean selectorIsShow = true;
    ...
    public ColorCoordinateListener colorCoordinateListener;
    public TouchPixelListener touchPixelListener;
    private long debounceDuration = 0;
    private Handler debounceHandler = new Handler();
    private Point snapPoint = new Point();
    private ActionMode actionMode = ActionMode.ALWAYS;
    public float[] mappedPoints = new float[]{0, 0};
    ...
    public ColorPickerPreferenceManager preferenceManager =
    ...
    private void getAttrs(AttributeSet attrs) {
        ...
        try {
            ...
            if (a.hasValue(R.styleable.ColorPickerView_selector_is_show)) {
                this.selectorIsShow =
                        a.getBoolean(R.styleable.ColorPickerView_selector_is_show, selectorIsShow);
            }
    ...
    private void onCreate() {
        setPadding(0, 0, 0, 0);
        palette = new PhotoView(getContext());
    ...
    protected void onCreateByBuilder(Builder builder) {
        ...
        if (builder.flagView != null) setFlagView(builder.flagView, false);
    ...
    public void receiveOnTouchEvent(MotionEvent event) {
        switch (event.getActionMasked()) {
            case MotionEvent.ACTION_DOWN:
                ...
                FadeUtils.fadeIn(selector);
                if(touchPixelListener!=null){
                    touchPixelListener.onPointDownSelected(mappedPoints[0],mappedPoints[1]);
                }
            case MotionEvent.ACTION_MOVE:
                break;
            case MotionEvent.ACTION_UP:
                selector.setVisibility(GONE);
                FadeUtils.fadeOut(selector);
                if(touchPixelListener!=null){
                    touchPixelListener.onPointUpSelected();
                }
        }
    }
...
//    }
    ...
    public void setSelectorIsShow(boolean selectorIsShow) {
        this.selectorIsShow = selectorIsShow;
    }
    ...
    public void setColorCoordinateListener(ColorCoordinateListener colorCoordinateListener) {
        this.colorCoordinateListener = colorCoordinateListener;
    }

    public void setTouchPixelListener(TouchPixelListener touchPixelListener) {
        this.touchPixelListener = touchPixelListener;
    }
    ...
    public void fireColorListener(@ColorInt int color, final boolean fromUser) {
        if (this.colorListener != null) {
            ...
            if (this.colorCoordinateListener != null) {
                colorCoordinateListener.onColorPointSelected(getWidth(), getHeight(), selectedPoint.x, selectedPoint.y);
            }
...
//            if (centerPoint.y - flagView.getHeight() > 0) {
                ...
                flagView.setX(centerPoint.x - flagView.getWidth() / 2f + selector.getWidth() / 2f);
                flagView.setY(centerPoint.y - flagView.getHeight() / 2f + selector.getHeight() / 2f);
...
//            }
     ...
     */
    public void setFlagView(@NonNull FlagView flagView, boolean isShow) {
        flagView.gone();
        if (isShow) {
            addView(flagView);
     ...
     * @param color color.
     */
    public void selectByHsvColor2(@ColorInt int color) throws IllegalAccessException {
        float[] hsv = new float[3];
        Color.colorToHSV(color, hsv);
...
//        notifyToFlagView(selectedPoint);
     ...
     * @param drawable palette drawable.
     */
    public void setPaletteDrawable(Drawable drawable, float scalex, float scaley) {
        removeView(palette);
        palette = new PhotoView(getContext());
        // 禁用图片缩放功能 (默认为禁用，会跟普通的ImageView一样，缩放功能需手动调用enable()启用)
        palette.disenable();
        palette.setScaleX(scalex);
        palette.setScaleY(scaley);
