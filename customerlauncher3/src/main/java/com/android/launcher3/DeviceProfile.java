...
//                (availableHeightPx - 4 * edgeMarginPx - folderBottomPanelSize) / inv.numFolderRows);
        folderCellWidthPx = (int) (iconSizePx + 2.5f * cellPaddingX);
        folderCellHeightPx = (int) (iconSizePx + 3 * cellPaddingY + folderChildTextSize*2.5f);
//        folderChildDrawablePaddingPx = Math.max(0,
//                (folderCellHeightPx - iconSizePx - folderChildTextSize) / 3);
        folderChildDrawablePaddingPx = 2;
...
//    }
    ...
    public Rect getWorkspacePadding(Rect recycle) {
        ...
        if (isVerticalBarLayout()) {
            ...
            if (isTablet) {
                ...
                // Pad the top and bottom of the workspace with search/launcher_hotseat bar sizes
    ...
    public Rect getAbsoluteOpenFolderBounds() {
        if (isVerticalBarLayout()) {
            // Folders should only appear right of the drop target bar and left of the launcher_hotseat
            ...
            // Folders should only appear below the drop target bar and above the launcher_hotseat
    ...
    /**
     * When {@code true}, the device is in landscape mode and the launcher_hotseat is on the right column.
     * When {@code false}, either device is in portrait mode or the device is in landscape mode and
     * the launcher_hotseat is on the bottom row.
     */
    ...
    public void layout(DeviceDJMainActivity deviceDJMainActivity, boolean notifyListeners) {
        FrameLayout.LayoutParams lp;
        boolean hasVerticalBarLayout = isVerticalBarLayout();
        final boolean isLayoutRtl = Utilities.isRtl(deviceDJMainActivity.getResources());
...
//        searchBar.setLayoutParams(lp);
        ...
        PagedView workspace = (PagedView) deviceDJMainActivity.findViewById(R.id.workspace);
