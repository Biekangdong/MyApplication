...
//biekangdong 文件夹图标排序规则
public class ClippedFolderIconLayoutRule implements FolderIcon.PreviewLayoutRule {

    static final int MAX_NUM_ITEMS_IN_PREVIEW = 4;
    private static final int MIN_NUM_ITEMS_IN_PREVIEW = 2;

    final float MIN_SCALE = 0.376f;
    final float MAX_SCALE = 0.376f;
    final float MAX_RADIUS_DILATION = 0.136f;
    final float ITEM_RADIUS_SCALE_FACTOR = 1.14f;
...
//    }
    ...
    private void getPosition(int index, int curNumItems, float[] result) {
        ...
        if (curNumItems == 4 && index == 3) {
            ...
            index = 3;
        }else if (curNumItems == 3 && index == 2) {
            index = 3;
        }

        // We bump the radius up between 0 and MAX_RADIUS_DILATION % as the number of items increase
        float radius = mRadius * (1 + MAX_RADIUS_DILATION * (4 -
                MIN_NUM_ITEMS_IN_PREVIEW) / (MAX_NUM_ITEMS_IN_PREVIEW - MIN_NUM_ITEMS_IN_PREVIEW));
        double theta = theta0 + index * (2 * Math.PI / 4) * direction;

        float halfIconSize = (mIconSize * scaleForNumItems(4)) / 2;
