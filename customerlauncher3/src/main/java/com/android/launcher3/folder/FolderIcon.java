...
public class FolderIcon extends FrameLayout implements FolderListener {
    ...
    private static ArrayList<PreviewItemDrawingParams> mDrawingParams = new ArrayList<PreviewItemDrawingParams>();
    private Drawable mReferenceDrawable = null;
    Paint mBgPaint = new Paint();
    private Alarm mOpenAlarm = new Alarm();
    public static FolderIcon icon;
    ...
    public static FolderIcon fromXml(int resId, DeviceDJMainActivity deviceDJMainActivity, ViewGroup group,
        ...
        DeviceProfile grid = deviceDJMainActivity.getDeviceProfile();

        icon = (FolderIcon) LayoutInflater.from(deviceDJMainActivity).inflate(resId, group, false);
        ...
        mIconSize = deviceDJMainActivity.getDeviceProfile().iconSizePx;
        Drawable mIcon = deviceDJMainActivity.getResources().getDrawable(R.drawable.launcher_trans_bg);
        mIcon.setBounds(0, 0, mIconSize, mIconSize);
        icon.mFolderName.getTextView().setCompoundDrawables(null, mIcon, null, null);
//        FrameLayout.LayoutParams lp = (FrameLayout.LayoutParams) icon.mFolderName.getLayoutParams();
//        lp.topMargin = grid.iconSizePx/2 + grid.iconDrawablePaddingPx;

        icon.setTag(folderInfo);
        icon.setOnClickListener(deviceDJMainActivity);
        icon.mInfo = folderInfo;
        icon.mDeviceDJMainActivity = deviceDJMainActivity;
        icon.setContentDescription(deviceDJMainActivity.getString(R.string.folder_name_format, folderInfo.title));
        Folder folder = Folder.fromXml(deviceDJMainActivity);
        folder.setDragController(deviceDJMainActivity.getDragController());
        ...
        icon.setOnFocusChangeListener(deviceDJMainActivity.mFocusHandler);
        return icon;
    }

    public static void setmFolderName() {
        icon.mFolderName.getTextView2().setText("");
        icon.mFolderName.getTextView().setGravity(Gravity.CENTER_HORIZONTAL);
        icon.mFolderName.setClickLongType(1);
        SpannableString spannableString1;
        if (TextUtils.isEmpty(mFolderInfo.title)) {
            spannableString1 = new SpannableString("New Group");
        } else {
            spannableString1 = new SpannableString(mFolderInfo.title);
        }
        ...
        SpannableString spannableString2 = new SpannableString("单元数量" + mFolderInfo.contents.size());
        ...
        icon.mFolderName.getTextView().setText(spannableStringBuilder);
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        float padding = (totalWidth - mIconSize) / 2f;
        mTouchDeleteBounds.set(padding + mIconSize - ScreenUtils.dip2px(mDeviceDJMainActivity, 10), 0, padding + mIconSize + ScreenUtils.dip2px(mDeviceDJMainActivity, 10), ScreenUtils.dip2px(mDeviceDJMainActivity, 24));
    ...
    private boolean willAcceptItem(ItemInfo item) {
        ...
        return (itemType == LauncherSettings.ITEM_TYPE_SHORTCUT &&
                !mFolder.isFull() && item != mInfo && !mInfo.opened);
    }

    public boolean acceptDrop(ItemInfo dragInfo) {
        final ItemInfo item = dragInfo;
        ArrayList<ShortcutInfo> shortcutInfoArrayList = mFolderInfo.contents;

        //文件夹是否包含同一个设备ID
        boolean isContainLightId = false;
        for (ShortcutInfo shortcutInfo : shortcutInfoArrayList) {
            if (dragInfo.LightId.equals(shortcutInfo.LightId)) {
                isContainLightId = true;
                break;
            }
        }

        return !mFolder.isDestroyed() && willAcceptItem(item) && !isContainLightId;
    }
    ...
    public void onDragEnter(ItemInfo dragInfo) {
        ...
        if (SPRING_LOADING_ENABLED && (dragInfo instanceof ShortcutInfo)) {
    ...
    OnAlarmListener mOnOpenListener = new OnAlarmListener() {
        public void onAlarm(Alarm alarm) {
            ...
            mDeviceDJMainActivity.openFolder(FolderIcon.this);

        }
    };

    public Drawable prepareCreate(final View destView) {
        Drawable animateDrawable = getTopDrawable((FrameLayout) destView);
    ...
    public void performDestroyAnimation(final View finalView, Runnable onCompleteRunnable) {
        Drawable animateDrawable = getTopDrawable((FrameLayout) finalView);
        computePreviewDrawingParams(animateDrawable.getIntrinsicWidth(),
    ...
    private void onDrop(final ShortcutInfo item, DragView animateView, Rect finalRect,
        ...
        if (animateView != null) {
            ...
            if (to == null) {
                to = new Rect();
                Workspace workspace = mDeviceDJMainActivity.getWorkspace();
...
//                      center[1] - animateView.getMeasuredHeight() / 2);
    ...
    private PreviewItemDrawingParams getFinalIconParams(PreviewItemDrawingParams params) {
        float iconSize = mDeviceDJMainActivity.getDeviceProfile().iconSizePx;
    ...
    public static class PreviewBackground {
        ...
        public void setup(DisplayMetrics dm, DeviceProfile grid, View invalidateDelegate,
            ...
            basePreviewOffsetY = previewPadding + grid.folderBackgroundOffset + Utilities.pxFromDp(10, dm);
...
//        }
        ...
        if (LauncherModel.CHANGESTATUESTYPE == 1) {
            // 画出原图像
            int mIconSize = mDeviceDJMainActivity.getDeviceProfile().iconSizePx;
            Bitmap bitmap = drawableToBitmap(getResources().getDrawable(R.mipmap.dj_delete));

            float padding = (totalWidth - mIconSize) / 2f;
            canvas.drawBitmap(bitmap, padding + mIconSize - ScreenUtils.dip2px(getContext(), 12), 0, null);
            canvas.save();
        }
    }
    ...
    public Bitmap drawableToBitmap(Drawable drawable) {
        int w = ScreenUtils.dip2px(mDeviceDJMainActivity, 24);
        int h = ScreenUtils.dip2px(mDeviceDJMainActivity, 24);
    ...
    private Drawable getTopDrawable(FrameLayout v) {
        Drawable d = ((TextView) v.getChildAt(0)).getCompoundDrawables()[1];
    ...
    private void updateItemDrawingParams(boolean animate) {
        ...
        for (int i = 0; i < mDrawingParams.size(); i++) {
            PreviewItemDrawingParams p = mDrawingParams.get(i);
            p.drawable = getTopDrawable((FrameLayout) items.get(i));
    ...
    public void onTitleChanged(CharSequence title) {
        mFolderInfo.title = title;
        setmFolderName();
        //mFolderName.getTextView().setText(title);
    ...
    private static final int BG_INTENSITY = 245;
    public static int borderColor = Color.argb(255, BG_INTENSITY, BG_INTENSITY, BG_INTENSITY);
    ...
    public boolean animateState(int newState) {
        if (mState != newState) {
            mState = newState;
            ...
            if (mState == PRESSED) {
                ...
                valueAnimator.addUpdateListener(new AnimatorUpdateListener() {
                    ...
                    public void onAnimationUpdate(ValueAnimator animation) {
                        ...
                        if (curValue >= 0.8f) {
                            //设置边框的宽度以及边框的颜色
                            borderColor = Color.parseColor("#2CCE38");
                            invalidate();
                        }

                        if (curValue <= 0.2f) {
                            borderColor = Color.argb(255, BG_INTENSITY, BG_INTENSITY, BG_INTENSITY);
                            invalidate();
                        }
                    }
                });
